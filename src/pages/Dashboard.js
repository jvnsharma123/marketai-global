      ...
      <div className="app-layout">
        ...
        <main className="main-content">
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '1rem', marginBottom: '1.5rem' }}>
            {[
              { label: 'Total Generated', value: recentCount, icon: '✦' },
              { label: 'Saved Content', value: savedCount, icon: '◈' },
              { label: 'Current Plan', value: profile ? (profile.plan.charAt(0).toUpperCase() + profile.plan.slice(1)) : '—', icon: '◷' },
              ...
              <div key={s.label} className="card" style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                <div style={{ width: 44, height: 44, background: '#eef2ff', borderRadius: 10, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.3rem', color: '#6366f1', flexShrink: 0 }}>{s.icon}</div>
                <div>
          ...
          <div className="card" style={{ marginBottom: '1.5rem' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.8rem' }}>
              ...
              <span style={{ fontSize: '0.85rem', color: '#64748b' }}>{used} / {planLimit === -1 ? '∞' : planLimit} generations</span>
            </div>
            ...
            {profile?.plan === 'free' && used >= 4 && (
              <div className="alert alert-info" style={{ marginTop: '1rem', marginBottom: 0 }}>
                You're almost out of free generations. <Link to="/pricing" style={{ fontWeight: 700 }}>Upgrade now →</Link>
              </div>
            )}
          </div>
          ...
          <div className="card" style={{ background: 'linear-gradient(135deg, #6366f1, #8b5cf6)', color: '#fff', border: 'none' }}>
            <h2 style={{ fontWeight: 800, marginBottom: '0.5rem' }}>Ready to create content?</h2>
            <p style={{ opacity: 0.85, marginBottom: '1.2rem', fontSize: '0.95rem' }}>Generate social posts, WhatsApp messages, ads and emails in one click.</p>
            <Link to="/generate" className="btn" style={{ background: '#fff', color: '#6366f1' }}>✦ Generate content now</Link>
          </div>
        </main>
      </div>
    </div>
